...
namespace MasterData
{
   public sealed class ImmutableBuilder : ImmutableBuilderBase
   {
        ...
        public void ReplaceAll(System.Collections.Generic.IList<MyData> data)
        {
            var newData = CloneAndSortBy(data, x => x.RandomId, System.StringComparer.Ordinal);
            var table = new MyDataTable(newData);
            memory = new MemoryDatabase(
                memory.FailTable,
                memory.ItemMasterTable,
                memory.ItemMasterEmptyValidateTable,
        ...
        public void RemoveMyData(string[] keys)
        {
            var data = RemoveCore(memory.MyDataTable.GetRawDataUnsafe(), keys, x => x.RandomId, System.StringComparer.Ordinal);
            var newData = CloneAndSortBy(data, x => x.RandomId, System.StringComparer.Ordinal);
            var table = new MyDataTable(newData);
            memory = new MemoryDatabase(
                memory.FailTable,
        ...
        public void Diff(MyData[] addOrReplaceData)
        {
            var data = DiffCore(memory.MyDataTable.GetRawDataUnsafe(), addOrReplaceData, x => x.RandomId, System.StringComparer.Ordinal);
            var newData = CloneAndSortBy(data, x => x.RandomId, System.StringComparer.Ordinal);
            var table = new MyDataTable(newData);
            memory = new MemoryDatabase(
                memory.FailTable,
